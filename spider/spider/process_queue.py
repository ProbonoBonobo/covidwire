...
class Article:
    odors = re.compile(r'(please|subscr|comment|social network|\||share|facebook|twitter|{|function\s?\(|click|advert|reached at|rights reserved|top stories|©|affiliate|Associated Press|writer|email|permission|curated by|delivered every|news stor|privacy|copyright|link)', re.IGNORECASE)
    ...
    def content(self):
        ...
        for node in nodes:
            if node and node.string and node.string.strip() and not re.search(self.odors, node.string):
                txt.append(normalize("NFKD", fix_text(node.string.replace("\xa0", " "))).strip())
        body = '\n'.join(txt)
        return body
    ...
    def summary(self):
        for candidate in self.lxml.xpath(UniversalSelector.summary):
            txt = normalize("NFKD", fix_text(str(candidate).replace("\xa0", " "))).strip()
...
if True:
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute(f"select  * from spiderqueue where random() < 0.3 order by lastmod desc limit {LIMIT};")
    ...
    for url, res in responses.items():
        ...
        try:
            ...
            row['mod_status'] = 'pending' if row['prediction'] == 'approved' else 'rejected'
            try:
                print(
                    magenta(json.dumps(row, indent=4, default=lambda x: x if not isinstance(x, (datetime.datetime, dict)) else str(x))))

...
audience_labels = [
    'international', 'city', 'regional', 'national', 'indefinite', 'state'
]
LIMIT = 1000
MAX_REQUESTS = 100
...
UniversalSelector = DotDict({
    "content":
    "div[class*='-content'] > p, div[class*='content-'] > p, section > p, div[class*='body'] > p, div[class*='article'] > p, div[itemprop*='article'] > p, div[class*='text'] > p, section div[class*='inner'] > p, #article-content p, div[class*='article-body'] p, div[class*='premium'] p, div[class*='story'] p, article.story-body p, div.body-text, article > p, article p, .postBody p, div[class*='main-content'] p, .inner p, #content p, .postBody",
    "title":
    "//meta[@name='twitter:title' or @property='og:title' or @name='title']/@content",
    "published_at":
    "//meta[contains(@property, 'article:published_time') or contains(@property, 'datePublished') or contains(@property, 'parsely-pub-date') or @property='tncms-access-version' or contains(@property,'pubDate') or contains(@name, 'ublished') or contains(@property, 'article_date') or contains(@property, 'sailthru.date') or contains(@property, 'article:publishedtoweb') or @name='date' or @name='last_updated_date']/@content | //time/text() | //*[contains(@class, 'time') or contains(@id, 'time') or contains(@aria-label, 'a.m') or contains(@aria-label, 'p.m.') or contains(@aria-label, '2020')]/text()",
    "summary":
    "//meta[@property='og:description' or @property='twitter:description' or contains(@name,'description')]/@content",
    "author":
    "//meta[contains(@property, 'author') or contains(@name, 'author') or @property='byline' or @property='article_author' or @property='og:sitename' or @property='twitter:site']/@content",
})
...
class Article:
    ...
    def __init__(self,
        ...
        self.url = url
        self.html = fix_text(html.replace(
            "\xa0", " ")) if fix_encoding_errors else html
    ...
    def published_at(self):
        ...
        if date is not NULL_DATE:
            print(
                f"Found a date: {guess.date}! Accuracy is to: {guess.accuracy}"
            )
        else:
            print(
    ...
    def content(self):
        ...
        for node in nodes:
            if (node and node.string and node.string.strip()
                    and not re.search(self.odors, node.string)):
                txt.append(
                    normalize("NFKD",
                              fix_text(node.string.replace("\xa0",
                                                           " "))).strip())
        body = "\n".join(txt)
        return body
    ...
    def title(self):
        for candidate in self.lxml.xpath(UniversalSelector.title):
            ...
            longest = list(sorted(re.split(r"\s[|-]\s", candidate),
                                  key=len))[-1]
    ...
    def summary(self):
        for candidate in self.lxml.xpath(UniversalSelector.summary):
            txt = normalize("NFKD",
                            fix_text(str(candidate).replace("\xa0",
                                                            " "))).strip()
...
if True:
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        rows = {
            row["url"]: row
            for row in cur.fetchall() if row["url"] not in seen
        }
    urls = list(rows.keys())
    ...
    for url, res in responses.items():
        ...
        if row["published_at"] == NULL_DATE and "lastmod" in row and row[
                "lastmod"]:
        ...
        try:
            tokens = list(
                [t.strip() for t in re.split(r"\s+", row["content"].lower())])
            ...
            row["mod_status"] = ("pending" if row["prediction"] == "approved"
                                 else "rejected")
            try:
                print(
                    magenta(
                        json.dumps(
                            row,
                            indent=4,
                            default=lambda x: x if not isinstance(
                                x, (datetime.datetime, dict)) else str(x),
                        )))

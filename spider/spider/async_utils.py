...
async def fetch_responses(urls: List[str], number_workers: int):
    ...
    async def token_issuer(token_sender: trio.abc.SendChannel,
                           number_tokens: int):
    ...
    async with trio.open_nursery() as nursery:
        async with token_receive_channel:
            nursery.start_soon(token_issuer, token_send_channel.clone(),
                               len(urls))
            for _ in range(number_workers):
                nursery.start_soon(worker, url_iterator,
                                   token_receive_channel.clone())

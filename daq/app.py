...
for options in [(row['state_name'], row['display_name']) for row in db.query("select display_name, state_name from uscities where population > 3000")]:
    suggestions.update(options)


# suggestions = ["San Francisco", "San Francisco County", "San Francisco Bay Area", "Oakland", "Berkeley", "San Jose", "Cupertino", "Contra Costa County", "Mountain View", "Alameda County"]  # generated by, i.e. query to db
                                                ...
                                                [
                                                    ...
                                                    dbc.Col(
                                                        dcc.Dropdown(
                                                            ...
                                                            id='audience-filters',
                                                            value=[
                                                                'local', 'regional', 'state', 'national', 'international', 'unbound'
                                                            ],
                                                            multi=True,
                                                        ),
                                                        width=8,
                                                    ...
                                                    #     [
                                                    dbc.Col(
                                                        dbc.Label("Password:"), width=1, className='hidden'
                                                    ),
                                                    dbc.Col(
                                                        dbc.Input(
...
app.layout = html.Div(
    [
        ...
        right_sidebar,
        dcc.Store(id='appstate',data={'history': [], 'hidden_weights': {}, 'id': None,
                                      "content": "",
                                      "title": "",
                                      "description": "",
                                      "url": "",
...
@app.callback(Output('time-sensitivity-container', 'className'),
              [Input('quality-score', 'value')])
def unhide_component(quality_score):
...
def render_page_content(submit, skip, undo, audience_filters, auth, score, time_sensitivity, audience_label, appstate, tags, location):
    ...
    # print(f"Count tokens called with {appstate}. Last modified {dt} seconds ago...")
    # if dt < 0.3:
    ...
    _kwargs = {'p': submit,
               ...
               "time_sensitivity": 0 if not score else time_sensitivity + 1,
    ...
    if appstate and 'url' in appstate:
        _kwargs.update({"url": appstate['url'], "title": appstate['title'], "name": appstate['name'], 'description': appstate['description']})
        if action == 'submit':
            appstate['history'].append(appstate['id'])
    ...
    res = requests.get("https://kevinzeidler.com/api/classified", params={"payload": payload}, timeout=10)
    if not res.ok:
        print(res.__dict__)
        raise dash.exceptions.PreventUpdate
    headline = ""
    ...
    try:
        ...
        print(res.__dict__)
        print(f"Bad result: {e.__class__.__name__} :: {e}")
        return dash.no_update
    ...
    nxt = results
    labeled_articles_count = nxt['labeled_articles_count']
    headline = nxt["title"]
...
# print(audience_graph)
    return [content, headline, image_url, description, publication, auth, audience_graph, approval_matrix, appstate, None, None, None, None, [], labeled_articles_count]
    # else:
    #     return dash.no_update
...
@app.callback(Output('list-suggested-inputs', 'children'),
              [Input('input-1', 'value')])
